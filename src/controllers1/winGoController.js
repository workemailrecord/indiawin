...
const rosesPlus = async (auth, money) => {
    ...
    const [user] = await connection.query('SELECT `phone`, `code`, `invite`, `user_level`, `total_money` FROM users WHERE token = ? LIMIT 1 ', [auth]);
    let userInfo = user[0];
    const [f1] = await connection.query('SELECT `phone`, `code`, `invite`, `rank`, `user_level`, `total_money` FROM users WHERE code = ? LIMIT 1 ', [userInfo.invite]);
        ...
        if (f1.length > 0) {
            ...
            for (let levelIndex = 1; levelIndex <= 6; levelIndex++) {
                ...
                if (true) {
                    ...
                    if (rosesF > 0) {
                        await connection.query('UPDATE users SET pending_balance = pending_balance + ?, roses_f = roses_f + ?, roses_today = roses_today + ? WHERE phone = ? ', [rosesF, rosesF, rosesF, infoF1.phone]);
...
const tradeCommission = async (req, res) => {
  try {
    // Fetch users with pending commission
    const [users] = await connection.execute(
      'SELECT * FROM `users` WHERE `pending_balance` > 0'
    );

    if (users.length === 0) {
      return res.json({ message: 'No users with pending commission.' });
    }
    
    // Get current timestamp
    const timeNow = Math.floor(Date.now() / 1000); // Assuming UNIX timestamp in seconds

    // Loop through users and update each one
    for (let user of users) {
      const { pending_balance, phone } = user;

      // Update user's money and reset pending_balance
      await connection.execute(
        'UPDATE `users` SET `money` = `money` + ?, `pending_balance` = 0 WHERE `phone` = ?',
        [pending_balance, phone]
      );

      // Generate unique commission ID
      const randomPart = Math.floor(10000 + Math.random() * 90000); // Random 5-digit number
      const commissionId = `cmn-${timeNow}-${randomPart}`;

      // Insert commission record
      const sql4 = `INSERT INTO commissions (commission_id, phone, from_user_phone, money, level, time) 
                    VALUES (?, ?, ?, ?, ?, ?)`;
      await connection.execute(sql4, [commissionId, phone, phone, pending_balance, 1, timeNow]);
    }

    // Return success response if done
    return res.json({ message: 'Commissions processed successfully.' });

  } catch (error) {
    // Return error message in JSON format
    return res.status(500).json({ message: 'Error processing commissions.', error: error.message });
  }
};
...
const betWinGo = async (req, res) => {
    try {
        ...
        if (check >= 0) {
            ...
            status = ?,
            time = ?`;
            await connection.execute(sql, [id_product, userInfo.phone, userInfo.code, userInfo.invite, period, userInfo.level, total, x, fee, 0, gameJoin, join, 0, timeNow]);
