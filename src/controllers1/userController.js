...
const constructTransactionsQuery = (
  ...
  const queries = {
    ...
    Commissions: {
      // Updated query to filter out today's records based on 'today' column format
      query: `SELECT commission_id AS id, SUM(money) AS money, 'positive' AS type, 'Commission' AS name, time 
              FROM commissions 
              WHERE phone = ? 
              AND DATE(today) <> CURDATE() 
              AND time >= ? AND time <=? 
              GROUP BY time`,
