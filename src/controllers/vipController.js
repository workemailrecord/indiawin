...
const releaseVIPLevel = async () => {
  try {
    ...
    for (let user of users) {
      ...
      // console.log(`Exp Points: ${expPoint}, Last VIP Level: ${lastVipLevel}`);
      ...
      if (currentVipLevel.level > lastVipLevel) {
        console.log(`Updating user ${phone} to VIP level ${currentVipLevel.level}`);
        await connection.execute(
        ...
        // Check if level-up reward has already been claimed
        const [existingLevelUpReward] = await connection.execute(
          "SELECT * FROM `claimed_rewards` WHERE `phone` = ? AND `reward_id` = ? AND `type` = ?",
          [
            phone,
            currentVipLevel.level,
        ...
        if (existingLevelUpReward.length === 0) {
        ...
        console.log(`Setting user ${phone} VIP level to ${currentVipLevel.level}`);
        await connection.execute(
      ...
      // Check if monthly reward has already been claimed
      const [existingMonthlyReward] = await connection.execute(
        "SELECT * FROM `claimed_rewards` WHERE `phone` = ? AND `reward_id` = ? AND `type` = ?",
        [
          phone,
          currentVipLevel.level,
      ...
      if (existingMonthlyReward.length === 0) {
        console.log(`Inserting monthly reward for user ${phone}`);
        await insertRewordClaim({
          rewardId: currentVipLevel.level,
    ...
    console.log("Error in releaseVIPLevel:", error);
  }
};

// Set interval to call releaseVIPLevel function every 5 seconds
setInterval(releaseVIPLevel, 5000);
