...
console.log(others);
  return res.status(200).json({
    ...
    data: {
      ...
      allowed_tabs : others.allowed_tabs,
    },
...
const constructTransactionsQuery = (
  ...
  const queries = {
    ...
    Commissions: {
      query: `SELECT commission_id AS id, SUM(money) AS money, 'positive' AS type, 'Commission' AS name, time FROM commissions WHERE phone = ? AND time >= ? AND time <=? GROUP BY commission_id, time`,
      count: `SELECT COUNT(*) AS totalCount FROM (SELECT commission_id, time FROM commissions WHERE phone = ? AND time >= ? AND time <=? GROUP BY commission_id, time) AS grouped`,
    },
    "Gift Vouchers": {
